...
IntervalTreeNode::IntervalTreeNode(Interval *newInterval)
    : storedInterval(newInterval), key(newInterval->GetLowPoint()), high(newInterval->GetHighPoint()), maxHigh(high) {}
IntervalTreeNode::~IntervalTreeNode() {}
Interval::Interval() {}
Interval::~Interval() {}
...
IntervalTree::IntervalTree() {
    ...
    recursionNodeStack = (it_recursion_node *)malloc(recursionNodeStackSize * sizeof(it_recursion_node));
...
TemplateStack<void *> *IntervalTree::Enumerate(int low, int high) {
...
#ifdef DEBUG_ASSERT
    Assert((recursionNodeStackTop == 1), "recursionStack not empty when entering IntervalTree::Enumerate");
#endif
    currentParent = 0;
    ...
    while (stuffToDo) {
        ...
        if (x->left->maxHigh >= low) {  // implies x != nil
            if (recursionNodeStackTop == recursionNodeStackSize) {
                ...
                recursionNodeStack = (it_recursion_node *)realloc(recursionNodeStack,
                                                                  recursionNodeStackSize * sizeof(it_recursion_node));
                if (recursionNodeStack == NULL) exit(1);
            }
...
#ifdef DEBUG_ASSERT
    Assert((recursionNodeStackTop == 1), "recursionStack not empty when exiting IntervalTree::Enumerate");
#endif
    return (enumResultStack);
}

int IntervalTree::CheckMaxHighFieldsHelper(IntervalTreeNode *y, const int currentHigh, int match) const {
    if (y != nil) {
...
void IntervalTree::CheckAssumptions() const { CheckMaxHighFields(root->left); }
