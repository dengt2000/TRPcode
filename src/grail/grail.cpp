    ...
    if (traversal == 0) {
        ...
        for (int i = 0; i < cnt; i++) {
            switch (type) {
                ...
                case 3:
                    customIndex[i] =
                        min(customIndex[i], (double)g[i].post->at(traversal - 1) - g[i].pre->at(traversal - 1));
                    break;
                case 4:
                ...
                case 5:
                    customIndex[i] =
                        min(customIndex[i], g[i].post->at(traversal - 1) - g[i].pre->at(traversal - 1) - g.tcs(i));
                    break;
            }
        }
    }
...
void Grail::fixedreverselabeling(Graph &tree, int traversal) {
    ...
    if (traversal % 2) reverse(roots.begin(), roots.end());
...
int Grail::fixedreversevisit(Graph &tree, int vid, int &pre_post, vector<bool> &visited, int traversal) {
    ...
    if (traversal % 2) reverse(el.begin(), el.end());
    EdgeList::iterator eit;
    int pre_order = tree.num_vertices() + 1;
    tree[vid].middle->push_back(pre_post);
    for (eit = el.begin(); eit != el.end(); eit++) {
...
int Grail::containsPP(int src, int trg) {
    ...
    if (POOL) {
        for (i = 0; i < dim; i++) {
            j = rand() % POOLSIZE;
            if (g[src].pre->at(j) > g[trg].pre->at(j)) return -1;
            if (g[src].post->at(j) < g[trg].post->at(j)) return -1;
            if (g[src].middle->at(j) < g[trg].post->at(j)) return 1;
        }
    } else {
        for (i = 0; i < dim; i++) {
            if (g[src].pre->at(i) > g[trg].pre->at(i)) return -1;
            if (g[src].post->at(i) < g[trg].post->at(i)) return -1;
            if (g[src].middle->at(i) < g[trg].post->at(i)) return 1;
        }
    }
    return 0;
}
        ...
        if (visited[*eit] != QueryCnt) {
            ...
            switch (res) {
                case 1:
                    ...
                    return true;
                case 0:
                    if (go_for_reachPP_lf(*eit, trg)) return true;
                    break;
                case -1:
#ifdef DEBUG
                    NegativeCut++;
        ...
        if (visited[*eit] != QueryCnt) {
            ...
            switch (res) {
                case 1:
                    ...
                    return true;
                case 0:
                    if (go_for_reachPP(*eit, trg)) return true;
                    break;
                case -1:
#ifdef DEBUG
                    NegativeCut++;
