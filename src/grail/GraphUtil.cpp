...
void GraphUtil::topo_leveler(Graph &g) {
    ...
    for (int i = 0; i < N; i++) topo_level(g, i);
}
...
void GraphUtil::topological_sort(Graph g, vector<int> &ts) {
    ...
    for (sit = roots.begin(); sit != roots.end(); sit++)
        if (!visited[*sit]) dfs(g, (*sit), preorder, postorder, visited);
...
// given start node
void GraphUtil::tarjan(Graph &g, int vid, int &index, unordered_map<int, pair<int, int> > &order, vector<int> &sn,
...
void GraphUtil::mergeSCC(Graph &g, int *on, vector<int> &reverse_topo_sort) {
    ...
    for (k = 0; k < origsize; k++) on[k] = indexmap[on[k]];
...
void GraphUtil::traverse(Graph &tree, int vid, int &pre_post, vector<bool> &visited) {
    ...
    for (eit = el.begin(); eit != el.end(); eit++) {
        pre_order = pre_post;
        pre_post++;
        if (!visited[*eit]) traverse(tree, *eit, pre_post, visited);
...
void GraphUtil::genRandomGraph(int n, double c, char *filename) {
    ...
    for (i = 0; i < n; i++) {
        for (j = 0; j < n; j++)
            if (i != j) {
                rand_num = rand() % 10000;
                if (rand_num < threshold) g.addEdge(i, j);
            }
    }
