...
int graph_io::writeGraph(graph_access& G, const std::string& filename,
    ...
    forall_nodes(G, node) {
        forall_out_edges(G, e, node) { f << (G.getEdgeTarget(e) + 1) << " "; }
        endfor f << "\n";
    }
    endfor
...
int graph_io::readGraphWeighted(graph_access& G, const std::string& filename) {
    ...
    while (std::getline(in, line)) {
        ...
        if (read_nw) {
            ...
            if (total_nodeweight >
                (long long)std::numeric_limits<NodeWeight>::max()) {
                std::cerr << "The sum of the node weights is too large (it "
                             "exceeds the node weight type)."
                          << std::endl;
                std::cerr << "Currently not supported. Please scale your node "
                             "weights."
                          << std::endl;
                exit(0);
            }
...
int graph_io::readDiGraph(graph_access& G, const std::string& filename) {
    ...
    if (nmbEdges > std::numeric_limits<int>::max() ||
        nmbNodes > std::numeric_limits<int>::max()) {
        std::cerr << "The graph is too large. Currently only 32bit supported!"
                  << std::endl;
        exit(0);
...
void graph_io::writePartition(graph_access& G, const std::string& filename) {
    ...
    forall_nodes(G, node) { f << G.getPartitionIndex(node) << "\n"; }
    endfor

        f.close();
}

int graph_io::readDiGraphFromPT(graph_access& G, ptree::Graph& graph) {
    std::cout << "开始转换OReach格式\n";
    long nmbNodes;
    long nmbEdges;
    nmbNodes = graph.num_vertices();
    nmbEdges = graph.num_edges();
