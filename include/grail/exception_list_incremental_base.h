...
class ExceptionListIncrementalBase : public ExceptionList {
   public:
    ...
    class ChildInterval : public Interval {
       public:
        int x, y;
        ChildInterval(int _x, int _y) : x(_x), y(_y) {}
    ...
    struct children_compare : public binary_function<Vertex, Vertex, bool> {
        bool operator()(const Vertex &x, const Vertex &y) {
            if (intervals[x].first == intervals[y].first) return intervals[x].second > intervals[y].second;
            return intervals[x].first < intervals[y].first;
        }
    } cc;

    void printStats(int dim) {
        for (int i = 0; i < dim; i++) cout << counts[i] << "\t";
        cout << endl;
        for (int i = 0; i < dim; i++) cout << times[i] << "\t";
        cout << endl;
    ...
    void getIntervals(Graph &org, int index) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            //			cout << "interval["<<i<<"]= ["<< intervals[org[i]].first << "," <<
            // intervals[org[i]].second
            //<<
            //"]"<< endl;
        ...
        //		for(int i=0; i< N;i++){
        //			cout << "interval["<<i<<"]= ["<< intervals[vlist[i]].first << "," <<
        // intervals[vlist[i]].second << endl;
        //		}
        ...
        */
        ...
        for (vit = vlist.begin(); vit != vlist.end(); vit++) {
            ...
            if (intervals[*vit].first <= result[index].first) {
                if (intervals[*vit].first != result[index].first) extra.push_back(result[index].first);
    ...
    void obtainDirectExceptions(Graph &g) {
        ...
        for (i = 0; i < vlist.size(); i++) {
            ...
            for (j = 0; j < openIntervals.size() - 1; j++) {
                //		cout << "j = " << j << endl;
                opencount++;
                results = (TemplateStack<VertexInterval *> *)intervalTree.Enumerate(openIntervals[j].first * 2 + 1,
            ...
            // The following is seperated from the loop, since some openIntervals arrays contain a dummy interval at the
            // end
            ...
            for (j = 0; j < extras.size(); j++) {
                results =
                    (TemplateStack<VertexInterval *> *)intervalTree.Enumerate(extras[j] * 2 + 1, extras[j] * 2 + 1);
                for (k = 0; k < results->Size(); k++) {
                    if ((*results)[k]->GetLowPoint() / 2 >= intervals[vlist[i]].first &&
                        (*results)[k]->GetHighPoint() / 2 <= intervals[vlist[i]].second) {
                        ...
                        if (t1 == el.end()) {
                            if (!LEVEL_FILTER ||
                                vlist[i].min_parent_level <=
                                    (*results)[k]
                                        ->_v.top_level)  // it is "<=" instead of "<" to be able to capture indirects
                                directs[vlist[i].id].insert((*results)[k]->_v.id);
                        }
                    }
                }
        ...
        for (i = 0; i < vlist.size(); i++) {
            ...
            for (t1 = el.begin(); t1 != el.end(); t1++) {
                for (setItr = directs[*t1].begin(); setItr != directs[*t1].end(); setItr++) {
                    if (used[*setItr] == i || (LEVEL_FILTER && vlist[i].top_level >= g[*setItr].top_level) ||
                    ...
                    for (t2 = el.begin(); t2 != el.end(); t2++) {
                        if (t1 != t2 &&
                            (intervals[g[*t2]].first <= intervals[g[*setItr]].first &&
   ...
   private:
    void obtainIncrementalExceptions(Graph &g, int cur) {
        ...
        for (i = 0; i < vlist.size(); i++) {
            ...
            for (setItr = lists[vlist[i].id].begin(); setItr != lists[vlist[i].id].end(); setItr++) {
                if (intervals[vlist[i]].first > intervals[g[*setItr]].first ||
                    ...
                    //						for(t1 = out.first; t1 != out.second ; t1++){
                    for (t1 = el.begin(); t1 != el.end(); t1++) {
                        if (g.incrementalContains(*t1, *setItr, cur) && lists[*t1].find(*setItr) == lists[*t1].end()) {
                            toBeRemoved.push_back(*setItr);
                            break;
                        }
                    }
        ...
        count = 0;
        for (i = 0; i < vlist.size(); i++) count += lists[i].size();
