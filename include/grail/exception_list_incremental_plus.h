    ...
    /*
            void obtainInitialExceptions(const Graph& g, GraphIndex& gi){
                    ...
                    for(i=0; i<vlist.size(); i++){
                            ...
                            for(j=0;j<openIntervals.size()-1;j++){
                                    opencount++;
                                    results = (TemplateStack<VertexInterval*>*)
       intervalTree.Enumerate( openIntervals[j].first*2+1,
       openIntervals[j].second*2-1);

                                    for(k=0; k < results->Size() ; k++){
                                            if((*results)[k]->GetLowPoint()/2 >=
       intervals[vlist[i]].first &&
                                                    (*results)[k]->GetHighPoint()/2
       <= intervals[vlist[i]].second )
                    //
       if(gi.contains(vlist[i],(*results)[k]->_v))
                                                    lists[vlist[i]].insert((*results)[k]->_v);
                                    }
                            }
                    // The following is seperated from the loop, since some
       openIntervals arrays contain a dummy interval at the end j =
       openIntervals.size()-1; if(openIntervals[j].first
       !=openIntervals[j].second-1){ opencount++; results =
       (TemplateStack<VertexInterval*>*) intervalTree.Enumerate(
                                                                            openIntervals[j].first*2+1,
       openIntervals[j].second*2-1);
       ...
       if(gi.contains(vlist[i],(*results)[k]->_v))
                            ...
                            for(j=0;j<extras.size();j++){
                                    results = (TemplateStack<VertexInterval*>*)
       intervalTree.Enumerate( extras[j]*2+1, extras[j]*2+1); results2 =
       (TemplateStack<VertexInterval*>*) it.Enumerate( extras[j]*2+1,
       extras[j]*2+1); for(k=0; k < results->Size() ; k++){
                                            if((*results)[k]->GetLowPoint()/2 >=
       intervals[vlist[i]].first &&
                                                            (*results)[k]->GetHighPoint()/2
       <= intervals[vlist[i]].second ){ for(t=0; t < results2->Size() ; t++){
                                                    //for(t1=out.first ;
       t1!=out.second; t1++){
                                                    //
       ...
       (*results)[k]->GetHighPoint()/2 <= intervals[*t1].second )
                                                            if((*results)[k]->GetLowPoint()
       >= (*results2)[t]->GetLowPoint() &&
                                                                            (*results)[k]->GetHighPoint()
       <= (*results2)[t]->GetHighPoint()) break;
                                                    }
                                                    if(t==results2->Size()){
                                                            lists[vlist[i]].insert((*results)[k]->_v);
                                                    }
                    ...
                    for(i=0;i<vlist.size();i++){
                            out = adjacent_vertices(vlist[i],g);
                            for(t1 = out.first; t1 != out.second ; t1++){
                                    for(setItr = lists[*t1].begin() ; setItr !=
       lists[*t1].end(); setItr++){
                    //
       if(lists[vlist[i]].find(*setItr)!=lists[vlist[i]].end()) break;
                                            if(used[*setItr]==i ||
       lists[vlist[i]].find(*setItr)!=lists[vlist[i]].end()) continue;
                                            used[*setItr]=i;
                                            flag = true;
                                            for(t2 = out.first; t2 != out.second ;
       t2++){ if(t1!=t2 && (intervals[*t2].first <= intervals[*setItr].first
                                                                                    && intervals[*t2].second >=
       intervals[*setItr].second)
                                                                                    && lists[*t2].find(*setItr) ==
       lists[*t2].end()){ flag = false; break;
                                                    }
                                            }
                                            if(flag){
                                                    count2++;
    ...
    void obtainIncrementalExceptions(Graph &g, int cur) {
        ...
        for (i = 0; i < vlist.size(); i++) {
            ...
            for (setItr = directs[vlist[i].id].begin(); setItr != directs[vlist[i].id].end(); setItr++) {
                ...
                /*				else{
                                                        for(t1 = out.first; t1 !=
                   out.second ; t1++){ if(gi.incrementalContains(*t1,*setItr,cur)
                                                                                                &&
                   lists[*t1].find(*setItr) == lists[*t1].end()){ toBeRemoved.push_back(*setItr); break;
                                                                }
                                                        }
                                                }
                */
        ...
        count = 0;
        for (i = 0; i < vlist.size(); i++) count += directs[i].size() + indirects[i].size();
        counts.push_back(count);
        cout << "count 1: " << count << endl;
    }
};
